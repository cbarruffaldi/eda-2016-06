...
public class ClosedHash <K, V> implements SimpleMap<K, V> {
    ...
    private Data<K, V>[] array;
        // se expande si la tabla está un dos tercios llena
    private int size;
    @SuppressWarnings("unchecked") public ClosedHash (int initialSize) {
    ...
    @SuppressWarnings("unchecked") @Override public void clear () {
    ...
    public Iterator<K> keyIterator () {
        ValueGetter<K> getter = new ValueGetter<K>() {
            @Override public K getValue (Data<K, V> n) {
                return n.key;
            }
        };
    ...
    @Override public Iterator<V> valueIterator () {
        ValueGetter<V> getter = new ValueGetter<V>() {
            @Override public V getValue (Data<K, V> n) {
                return n.value;
            }
        };
