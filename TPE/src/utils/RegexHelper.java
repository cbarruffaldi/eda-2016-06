...
public class RegexHelper implements InputConstraints {
    ...
    private static final String days = "(Lu|Ma|Mi|Ju|Vi|Sa|Do)(-(Lu|Ma|Mi|Ju|Vi|Sa|Do))*";
        // "Lu" o bien "Lu-Mi-Ju", etc
    private static final String airportName = "[a-zA-Z]{" + AIRPORT_NAME_LENGHT + "}";
    private static final String twentyFourHourFormat = "([01]?[0-9]|2[0-3]):[0-5][0-9]";
        //00:00 - 23:59
    private static final String hours = "([0-9]+h)?[0-5]?[0-9]m"; // [xxh]xxm
    private static final String realPositiveNum = "[0-9]+(\\.[0-9]+)?";

    private static final String latitude = "-?([0-8]?[0-9](\\.[0-9]+)?|90(\\.0+)?)";
        //-90.000 - 90.0000
    private static final String longitude = "-?(([0-9]|1[0-7])?[0-9](\\.[0-9]+)?|180(\\.0+)?)";
        //-180.000 - 180.0000
    ...
    private static final String routeFormat =
        "src=" + airportName + spc + "dst=" + airportName + spc + "priority=(ft|pr|tt)"
            + "( weekdays=" + days + ")?";

    private static String createFlightFormat (String separator) {
        return airlineName + separator + number + separator + days + separator + airportName
            + separator + airportName +
